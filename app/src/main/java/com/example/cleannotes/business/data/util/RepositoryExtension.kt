...
import com.example.cleannotes.business.data.cache.CacheConstants.CACHE_TIMEOUT
...
import com.example.cleannotes.business.data.network.NetworkConstants.NETWORK_TIMEOUT
...
suspend fun <T> safeCacheCall(
    dispatcher: CoroutineDispatcher,
    cacheCall: suspend () -> T? // takes suspend fn as input which return, say 'T', then safeCacheCall will return CacheResult<T>
): CacheResult<T?> {
    return withContext(dispatcher) {
        try {
            // throws TimeoutCancellationException
            withTimeout(CACHE_TIMEOUT){
                CacheResult.Success(cacheCall.invoke()) // cacheCall.invoke() - calling cacheCall method passed in safeCacheCall
            }
        } catch (throwable: Throwable) {
            throwable.printStackTrace()
            when (throwable) {
